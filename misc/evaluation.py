    ...
    for track in track_list:
        if state == None or track.state == state:  # plot e.g. only confirmed tracks
            ...
            rec = plt.Rectangle(-point_of_rotation, width=w, height=l,
                                color=col, alpha=0.2,
                                transform=Affine2D().rotate_around(*(0, 0), -yaw) + Affine2D().translate(-y,
                                                                                                         x) + ax.transData)
            ax.add_patch(rec)
            ...
            if track.state == 'initialized':
                ax.scatter(float(-track.x[1]), float(track.x[0]), color=col, s=80, marker='x',
        ...
        for i, result_dict in enumerate(manager.result_list):
            ...
            for label, valid in zip(label_list[0], label_list[1]):
                ...
                if valid:
                    ...
                    if label.box.center_x > configs_det.lim_x[0] and label.box.center_x < configs_det.lim_x[
                        1] and label.box.center_y > configs_det.lim_y[0] and label.box.center_y < configs_det.lim_y[1]:
